...
function N8nIntegrationPanel({ onToast }: { onToast: (t: string, tone?: "ok" | "warn") => void }) {
  ...
  const [baseUrl, setBaseUrl] = React.useState("https://jacobworkinai.app.n8n.cloud");
  const [saving, setSaving] = React.useState(false);
  const [configured, setConfigured] = React.useState(false);
  React.useEffect(() => {
    fetch("/api/n8n/status").then(r => r.json()).then((d: any) => { setConfigured(d?.configured ?? false); if (d?.baseUrl) setBaseUrl(d.baseUrl); }).catch(() => {});
  }, []);
  const save = async () => {
    if (!apiKey.trim()) { onToast("API key required.", "warn"); return; }
    setSaving(true);
    try {
      const r = await fetch("/api/config/n8n", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ api_key: apiKey.trim(), base_url: baseUrl.trim() }) });
      const d = await r.json() as any;
      onToast(d?.message ?? "n8n configured.", "ok"); setConfigured(true); setApiKey("");
    } catch { onToast("Failed to save n8n config.", "warn"); }
    setSaving(false);
  };
  return (
    <>
      {configured && <div className="v2-set__row"><span className="v2-set__row-label">Status</span><span className="v2-set__row-value"><span className="v2-set__dot v2-set__dot--ok" /> Connected</span></div>}
      <div className="v2-set__field"><label className="v2-set__field-label">Instance URL</label><input className="v2-set__input" value={baseUrl} onChange={e => setBaseUrl(e.target.value)} placeholder="https://your-instance.app.n8n.cloud" /></div>
      <div className="v2-set__field"><label className="v2-set__field-label">API Key</label><input className="v2-set__input" type="password" value={apiKey} onChange={e => setApiKey(e.target.value)} placeholder="From n8n Settings → API" /><p className="v2-set__hint">In n8n: Settings → API → Create an API Key</p></div>
      <div style={{ display: "flex", justifyContent: "flex-end" }}><button type="button" className="v2-set__btn v2-set__btn--primary" onClick={save} disabled={saving}>{saving ? "Saving…" : configured ? "Update n8n" : "Connect n8n"}</button></div>
    </>
  );
}
...
function FacebookIntegrationPanel({ onToast }: { onToast: (t: string, tone?: "ok" | "warn") => void }) {
  ...
  const [configured, setConfigured] = React.useState(false);
  React.useEffect(() => {
    fetch("/api/meta/status").then(r => r.json()).then((d: any) => { setConfigured(d?.connected ?? false); if (d?.pageId) setPageId(d.pageId); }).catch(() => {});
  }, []);
  const save = async () => {
    if (!pageId.trim() || !token.trim()) { onToast("Page ID and access token required.", "warn"); return; }
    setSaving(true);
    try {
      const r = await fetch("/api/meta/credentials", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ accessToken: token.trim(), adAccountId: "", pageId: pageId.trim() }) });
      const d = await r.json() as any;
      onToast(d?.ok ? "Facebook page connected." : "Failed to save.", d?.ok ? "ok" : "warn");
      if (d?.ok) { setConfigured(true); setToken(""); }
    } catch { onToast("Failed to save Facebook config.", "warn"); }
  ...
  return (
    <>
      {configured && <div className="v2-set__row"><span className="v2-set__row-label">Status</span><span className="v2-set__row-value"><span className="v2-set__dot v2-set__dot--ok" /> Page connected</span></div>}
      <div className="v2-set__field"><label className="v2-set__field-label">Facebook Page ID</label><input className="v2-set__input" value={pageId} onChange={e => setPageId(e.target.value)} placeholder="123456789" /></div>
      <div className="v2-set__field"><label className="v2-set__field-label">Page Access Token</label><input className="v2-set__input" type="password" value={token} onChange={e => setToken(e.target.value)} placeholder="EAAxxxxx..." /><p className="v2-set__hint">From <a href="https://developers.facebook.com/tools/explorer" target="_blank" rel="noreferrer" style={{ color: "var(--ok)" }}>Meta Graph API Explorer</a> — generate a Page Access Token.</p></div>
      <div style={{ display: "flex", justifyContent: "flex-end" }}><button type="button" className="v2-set__btn v2-set__btn--primary" onClick={save} disabled={saving}>{saving ? "Saving…" : configured ? "Update Facebook" : "Connect Facebook"}</button></div>
    </>
  );
}
...
function NvidiaIntegrationPanel({ onToast }: { onToast: (t: string, tone?: "ok" | "warn") => void }) {
  ...
  const [configured, setConfigured] = React.useState(false);
  React.useEffect(() => {
    fetch("/api/videogen/status").then(r => r.json()).then((d: any) => { setConfigured(d?.nvidiaConfigured ?? false); }).catch(() => {});
  }, []);
  const save = async () => {
    if (!apiKey.trim()) { onToast("API key required.", "warn"); return; }
    setSaving(true);
    try {
      const r = await fetch("/api/config/nvidia", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ api_key: apiKey.trim() }) });
      const d = await r.json() as any;
      onToast(d?.message ?? "NVIDIA key saved.", "ok"); setConfigured(true); setApiKey("");
    } catch { onToast("Failed to save NVIDIA key.", "warn"); }
    setSaving(false);
  };
  return (
    <>
      {configured
        ? <div className="v2-set__row"><span className="v2-set__row-label">Status</span><span className="v2-set__row-value"><span className="v2-set__dot v2-set__dot--ok" /> API key configured</span></div>
        : <div className="v2-set__row"><span className="v2-set__row-label">Status</span><span className="v2-set__row-value"><span className="v2-set__dot v2-set__dot--warn" /> Using Pollinations fallback</span></div>}
      <div className="v2-set__field"><label className="v2-set__field-label">NVIDIA API Key</label><input className="v2-set__input" type="password" value={apiKey} onChange={e => setApiKey(e.target.value)} placeholder="nvapi-xxxxxxx" /><p className="v2-set__hint">Free tier at <a href="https://build.nvidia.com" target="_blank" rel="noreferrer" style={{ color: "var(--ok)" }}>build.nvidia.com</a></p></div>
      <div style={{ display: "flex", justifyContent: "flex-end" }}><button type="button" className="v2-set__btn v2-set__btn--primary" onClick={save} disabled={saving}>{saving ? "Saving…" : configured ? "Update NVIDIA Key" : "Save NVIDIA Key"}</button></div>
    </>
  );
}
